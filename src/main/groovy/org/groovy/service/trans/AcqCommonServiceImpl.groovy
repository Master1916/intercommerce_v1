...
public class AcqCommonServiceImpl implements AcqCommonService {
    ...
    public Object sale(HttpServletRequest request, MultipartFile signature) {
        ...
        if (!merchant || merchant.enabled == 0) {
        ...
        //校验商户是否被关停
        if(4 == bankAccount.is_verified as int){
            return ResponseUtil.failResponse("MERCHANT_CLOSE_DOWN", "商户已关停，详情咨询代理商");
        }
        ...
        boolean tradeFlag = param.tradeFlag? BooleanUtils.toBoolean(param.tradeFlag as String) :false
        boolean tradeSecFlag = param.tradeSecFlag? BooleanUtils.toBoolean(param.tradeSecFlag as String) :false
        //判断是否D0业务交易
        if (tradeFlag) {
            ...
            //是否是秒到账
            if (tradeSecFlag) {
                merchant = merchantDao.findCmMerchantById(tradeMerchant.merchant_id_dzero_second);
                if (!merchant || merchant.enabled == 0) {
                    return ResponseUtil.failResponse("MERCHANT_NOT_EXIST", "不支持及时付(0.49费率)");
                }
                terminal = terminalDao.findTerminalByMerchantId(tradeMerchant.merchant_id_dzero_second);
            } else {
                merchant = merchantDao.findCmMerchantById(tradeMerchant.merchant_id_tzero);
                if (!merchant || merchant.enabled == 0) {
                    return ResponseUtil.failResponse("MERCHANT_NOT_EXIST", "不支持及时付");
                }
                terminal = terminalDao.findTerminalByMerchantId(tradeMerchant.merchant_id_tzero);
            }
        }

        //TODO 密钥处理 (MAC/PIN/TRACK)
        //mac处理
        ...
        if (!cardbin) {
            return Commons.fail(null, '15', '不支持的卡');
        }
        if ("credit".equalsIgnoreCase(cardbin.card_type as String) && (param.tradeFlag || param.tradeSecFlag)) {
            //如果是信用卡则必须是绑定的信用卡
            def cardbind = dictionaryDao.findBankCardByUserIdAndCardNo(user.user_id as String, cardHolder.pan as String)
            if (!cardbind) {
                log.info "信用卡不是绑定成功的卡 ：" + cardHolder.pan
                return Commons.fail(null, 'CARD_NOT_EFFECTIVE', '请使用已经绑定并且校验成功的信用卡')
            }
        }
        ...
        //金额
        long amount = param.amount as long;
        //交易重复校验
        def _tcount = transDao.queryTrans(batch, trace, terminal.terminal_no as String, merchant.merchant_no as String);
        if (_tcount > 0) {
            return Commons.fail(null, '94', '重复交易')
        }
        if (!signature) {
            return Commons.fail(null, 'ILLEGAL_ARGUMENT', '未发现签名图片')
        }
        //签名
        def signatureUri = "${now.format(Constants.DATE_FORMAT_YMD)}${terminal.terminal_no}${batch}${trace}"

        def checkResult = AuthUtil.checkImg(signature);
        if (!checkResult?.isSuccess) {
            return Commons.fail(null, checkResult?.code);
        }

        def success = AuthUtil.uploadSignImg(signature, signatureUri,)?.isSuccess
        if (!success) {
            return Commons.fail(null, 'ILLEGAL_ARGUMENT', '签名图片保存失败')
        }
        def av = ValidateUtil.versionParse(param.appVersion as String);
        if (!av) {
            return Commons.fail(null, 'ILLEGAL_ARGUMENT', '无效的AppVersion')
        }
        DictionaryDao dd = new DictionaryDao();
        def p = dd.findProductByName(av.model as String);
        if (!p) {
            return Commons.fail(null, 'ILLEGAL_ARGUMENT', '暂时不支持该产品');
        }
        def crt = checkRiskTrans(String.valueOf(amount), cardHolder.pan, terminal.terminal_no as String, p.id as String, termModel.id as String);
        if (crt) {
            return Commons.fail(null, crt[0], crt[1]);
        }

        //及时付交易处理
        if (tradeFlag) {
            param.pin = pin;
            param.signatureUri = signatureUri;
            def cdza
            //秒到账交易处理
            if (tradeSecFlag) {
                cdza = checkDayZeroSecAmount(trace, batch, merchant.agency_id as String, merchant.id as String, amount, cardHolder, cardbin, param);
                if (!cdza) {
                    return Commons.fail(null, 'ILLEGAL_ARGUMENT', '及时付(0.49费率)未完成');
                }
                return cdza;
            } else {
                cdza = checkDayZeroAmount(trace, batch, merchant.agency_id as String, merchant.id as String, amount, cardHolder, cardbin, param);
                if (!cdza) {
                    return Commons.fail(null, 'ILLEGAL_ARGUMENT', '及时付未完成');
                }
            }
            return cdza;
        }
        ...
        //上行交易流水预存
        def trans = transDao.addWsTrans(amount, batch, trace, cardHolder.pan, param.appVersion, param.currency, param.position, signatureUri, terminal, ksnNo, cardbin)
        try {
            //构建报文并且接受响应报文
            ISOMsg resultResponse = TransUtil.sendAndRecive(TransUtil.packageSaleMessage(batch, trace, cardHolder, amount, pin, terminal.merchant_no as String, terminal.terminal_no as String, param.cardSerialNum as String, param.icData as String))
            trans.comp_status = 1
            //TODO 后处理
            ...
            if (code == '00') {
                ...
                return Commons.success(res);
            } else {
                String errorMessage = Constants.trans_error_code.get(code) ?: code + ":未知错误";
                return Commons.fail([reqNo: param.reqNo ?: null, script: f55 ?: '', resultCode: code], code, errorMessage);
            }
        } finally {
            trans.comp_status = 2
